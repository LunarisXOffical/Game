...
-- Check if key already exists in getgenv().SCRIPT_KEY
if getgenv().SCRIPT_KEY and type(getgenv().SCRIPT_KEY) == "string" and #getgenv().SCRIPT_KEY > 0 then
    ...
    if validation and validation.valid then
        ...
        -- Immediately load the script if key is valid
        ...
        return  -- Exit the script since we're done
    else
        print("⚠ Existing SCRIPT_KEY is invalid, will show UI...")
        getgenv().SCRIPT_KEY = nil  -- Clear invalid key
    end
end

-- Only show UI if not already validated
if not validated then
    -- Your provided Junkie UI code
    ...
    -- MODIFIED Build function to handle our validation logic
    local function Build()
        ...
        -- Modified redeem button to track attempts
        local attempts = 0
        redeem.MouseButton1Click:Connect(
            function()
                attempts = attempts + 1
                if attempts > maxAttempts then
                    ToastSystem.Create(screen, "Too many attempts! Restart the script.", "error")
                    return
                end
                
                local key = box.Text:upper()
                if #key < 5 then
                    ToastSystem.Create(screen, "Please enter a valid key", "warning")
                    return
                end
                ...
                if result.valid then
                    ...
                    ToastSystem.Create(screen, result.message or "Invalid key (Attempts left: " .. attemptsLeft .. ")", "error")
    ...
    -- Modified key checking logic
    local savedKey = loadVerifiedKey()
    local keyToCheck = savedKey
    
    -- First check getgenv().SCRIPT_KEY (already checked above)
    -- Then check saved file key
    if not validated and keyToCheck then
