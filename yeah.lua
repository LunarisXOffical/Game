...
local mainScriptUrl = "https://raw.githubusercontent.com/LunarisXOffical/Game/refs/heads/main/ithinkyes.lua"

-- Function to save/load keys
...
-- FUNCTION 1: Check existing keys first
local function checkExistingKeys()
    print("🔍 Checking for existing keys...")
    
    -- Priority 1: Check getgenv().SCRIPT_KEY
    if getgenv().SCRIPT_KEY and type(getgenv().SCRIPT_KEY) == "string" then
        local key = getgenv().SCRIPT_KEY:gsub("%s+", ""):upper()
        if #key > 5 then
            print("📋 Found key in getgenv().SCRIPT_KEY:", string.sub(key, 1, 10) .. "...")
            ...
            if success and result and result.valid then
                print("✅ Key from getgenv() is VALID!")
                saveKey(key)
                return true, key
            else
                print("❌ Key from getgenv() is invalid")
...
-- Check for existing valid keys
local hasValidKey, validKey = checkExistingKeys()
...
print("🖥️ Showing Junkie UI...")

-- Your provided Junkie UI code starts here
    ...
    redeem.MouseButton1Click:Connect(
        function()
            ...
            if result.valid then
                -- SAVE TO BOTH getgenv().SCRIPT_KEY AND FILE
