...
-- This uses YOUR exact key system UI code

-- First, check if we already have a valid key in getgenv().SCRIPT_KEY
if getgenv().SCRIPT_KEY and type(getgenv().SCRIPT_KEY) == "string" then
    -- Load Junkie SDK to check the key
    ...
    if success and Junkie then
        ...
        if checkSuccess and type(checkResult) == "table" and checkResult.valid then
            print("✅ Using existing getgenv().SCRIPT_KEY")
...
-- If no valid key found, load YOUR key system UI
print("🖥️ Loading Junkie Key System UI...")

-- This is YOUR key system UI code with the proper fixes
...
-- THIS IS THE MAIN BUILD FUNCTION FROM YOUR KEY SYSTEM UI
local function Build()
	...
	-- FIXED REDEEM BUTTON: Proper error handling
	redeem.MouseButton1Click:Connect(
		function()
			...
			-- Use pcall to safely check the key
			...
			if not success then
				SetStatus("error")
				ToastSystem.Create(screen, "API Error: " .. tostring(result), "error")
				return
			end
			
			-- Check if result is a table and has valid property
			if type(result) == "table" and result.valid then
				...
				SetStatus("error")
				ToastSystem.Create(screen, type(result) == "table" and result.message or "Invalid key", "error")
			end
		end
	)
	...
	getKey.MouseButton1Click:Connect(
		function()
			...
			if success and link and setclipboard then
				...
				ToastSystem.Create(screen, "Failed to get key link", "error")
			end
		end
	)
	...
	paste.MouseButton1Click:Connect(
		function()
			ToastSystem.Create(screen, "Paste functionality not supported in Roblox", "warning")
		end
	)
...
if savedKey then
    print("🔍 Checking saved key from file...")
    ...
    if success and type(result) == "table" and result.valid then
...
print("📱 Building Junkie Key System UI...")
Build()

-- Wait for key validation (optional)
